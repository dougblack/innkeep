...
from innkeep.embeds import CardImage, CardText
from innkeep import cached
from innkeep.cached import CARDS

TOKEN = os.environ.get('INNKEEP_TOKEN')
QUERY_PATTERN = re.compile('\[\[([^\]]*)\]\]')

bot = commands.Bot(command_prefix='!', description='Hearthstone bot')

last_refresh = None


@bot.group(pass_context=True)
async def innkeep(ctx):
    """
    Creates a command group. !innkeep <command> is the usage pattern
    for all innkeep commands.
    """
    pass
...
async def help(*_):
    await bot.say(
        ...
        '!innkeep refresh - Refresh card cache.```'
    )
...
def exact_match(query, cards):
    ...
    Innkeep by using its exact name.
    """
...
def fuzzy_match(query, cards):
    ...
    score is less than 50 to prevent seemingly random responses from Innkeep.
    """
    # Fuzzy match over the card pool.
    results = process.extract(query, cards.keys(), limit=1, scorer=fuzz.token_set_ratio)
    if not results:
        return None
    # If score is less than 50, ignore this result. Innkeep will return no
    # embed in this case.
...
async def on_message(message):
    ...
    `!innkeep help` and `!innkeep refresh` commands a chance to fire, as
    ...
    for query in queries:
        ...
        if not CARDS:
            await bot.send_message(message.channel, f'My card pool is empty. https://hearthstonejson.com might be down.')
